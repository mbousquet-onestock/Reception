      ...
      >
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-3">
            <label className="text-sm font-bold text-gray-700 uppercase tracking-wider">Motif du problème</label>
          ...
          <div className="space-y-3">
            <label className="text-sm font-bold text-gray-700 uppercase tracking-wider">Quantité concernée</label>
            <div className="flex items-center gap-4">
              <button 
                onClick={() => setQty(Math.max(1, qty - 1))}
                className="w-12 h-12 rounded-xl border-2 border-gray-100 flex items-center justify-center text-2xl font-bold text-gray-400 hover:border-gray-200 hover:text-gray-600 transition-all"
              >
                -
              </button>
        ...
        <div className="p-6 bg-gray-50 flex gap-3">
          <button 
            onClick={onClose}
            className="flex-grow py-3 px-4 rounded-xl font-bold text-gray-500 hover:bg-gray-200 transition-colors"
          >
            Annuler
          </button>
          <button 
            onClick={() => onSave(reason, qty)}
            className="flex-grow py-3 px-4 rounded-xl font-bold bg-[#2563EB] text-white shadow-lg shadow-blue-200 active:scale-95 transition-all"
          >
            Confirmer
          </button>
        </div>
...
const TransferItem: React.FC<TransferItemProps> = ({ item, status, onUpdateDiscrepancy }) => {
  ...
  return (
    <div className="flex items-start gap-4 p-4 border-t border-gray-100 bg-white">
      ...
      <div className="flex-grow min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          {isDiscrepancy && (
            <span className="text-[11px] font-bold text-red-500 bg-red-50 px-1.5 py-0.5 rounded uppercase tracking-tight">
          ...
          <div className="flex flex-col items-end gap-2">
            {!isDiscrepancy ? (
              ...
              >
                ...
                <span className="text-[#1E293B] font-bold text-[14px]">Problème</span>
              </button>
            ) : (
      ...
      >
        ...
        <div className="flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
          {transfer.status === 'Reçu' ? (
            <div className="flex items-center gap-1.5 bg-[#1ABC9C] text-white px-3 py-1.5 rounded-md text-[14px] font-medium shadow-sm">
              <Check size={16} />
              <span>Reçu</span>
            </div>
          ) : (
            <button 
              onClick={onReceive}
              className="flex items-center gap-1.5 bg-[#EBF5FF] text-[#2563EB] px-3 py-1.5 rounded-md text-[14px] font-medium shadow-sm border border-[#BFDBFE] hover:bg-[#DBEAFE] active:scale-95 transition-all"
            >
              <Clock size={16} />
              <span>À réceptionner</span>
            </button>
          )}
          <div className="text-gray-400 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] font-sans">
      {/* Search Bar */}
      <div className="p-3 max-w-2xl mx-auto sticky top-0 z-10 bg-[#F8F9FA]/80 backdrop-blur-sm">
